...
import ValComp from "./ValidationComp/ValidationComp";
...
class App extends Component {
  ...
  deleteItemHandler = index => {
    const stringArray = [...this.state.textArray];
    stringArray.splice(index, 1);
    this.setState({ textArray: stringArray });
  };
  ...
  render() {
    return (
      <div className="App">
        ...
        <input type="text" onChange={event => this.getText(event)} value={this.state.textArray.join('')} />
        <ValComp text={this.state.textArray} />
        {this.state.textArray.map((char, index) => {
          return (
            <CharComp text={char} click={() => this.deleteItemHandler(index)} />
          );
        })}
      </div>
    );
